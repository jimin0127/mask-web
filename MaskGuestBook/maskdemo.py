...
from MaskGuestBook.views import filePath
...
class timer(threading.Thread):
    ...
    def run(self):
        ...
        cv2.imwrite(self.fileName, self.camera.frame)
...
class camera(threading.Thread):
    ...
    def run(self):
        ...
        # 이미지를 날짜 이름으로 미리 저장해놓고  날짜 이름을 view로 view에서 html에 날짜 이름을 넘겨주고 html에서 날짜이름으로 이미지 불러옴
